...
def main():
    ...
    try:
        while True:
            if active_mode is None:
                ...
                if command.lower() in ("exit", "close"):
                    ...
                    # Parse switch command with strict parameter validation
                    parts = command.lower().split()
                    if len(parts) == 1:
                        # Plain "switch" command - toggle to the other mode
                        active_mode = "notes"
                    elif len(parts) == 2:
                        if parts[1] == "1":
                            active_mode = "contacts"
                            ...
                            print(Fore.RED + f"⚠️ Невірний параметр '{parts[1]}'. Використовуйте 'switch 1' або 'switch 2'.")
                            continue
                    else:
                        print(Fore.RED + "⚠️ Невірна команда. Використовуйте 'switch', 'switch 1' або 'switch 2'.")
                        continue
                    continue
        ...
        # Handle KeyboardInterrupt gracefully without showing traceback
        print(Fore.GREEN + "\n👋 До побачення!")
        return
